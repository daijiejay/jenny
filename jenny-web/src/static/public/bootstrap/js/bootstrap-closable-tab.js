var closableTab = {
	...
	addTab:function(tabItem){ //tabItem = {id,name,url,closable}
		...
		if(!$('#'+id)[0]){
			...
			if(tabItem.closable){
				li_tab = li_tab + '<i class="fa fa-remove small" tabclose="'+id+'" style="position: absolute;right:4px;top: 4px;"  onclick="closableTab.closeTab(this)"></i></a></li> ';
			}else{
				li_tab = li_tab + '</a></li>';
			}
		
		 	var tabpanel = '<div role="tabpanel" class="tab-pane" id="'+container+'" style="width: 100%;">'+
	    					  '<iframe src="'+tabItem.url+'" menuId="'+tabItem.id+'" id="tab_frame_2" frameborder="0" style="overflow-x: hidden; overflow-y: hidden;width:100%;"  onload="changeFrameHeight(); frameLoad(this);"></iframe>'+
	    				   '</div>';
