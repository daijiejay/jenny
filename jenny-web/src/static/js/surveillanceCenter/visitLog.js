	...
	function loadApiInfo(param) {
		request('get', '', '/api-focus/api-docs?group='+param, '', function(result) {
			...
			jQuery.each(documentation.tags, function(index, controller) {
				...
				controllerStr += '	<span class="inbox-leftbar-category clearfix gallery">';
				controllerStr += '		<a href="javascript:;" data-toggle="collapse" data-target="#folders" class="category-heading">'+controller.description+'<span class="badge badge-warning">3</span></a>';
				controllerStr += '	</span>';
				...
				jQuery.each(documentation.paths, function(apiKey, api) {
					jQuery.each(api, function(methodKey, method) {
						if (method.tags && method.tags[0] == controller.name) {
							...
							$('.collapsible-menu .collapse[name="'+controller.name+'"]').append(methodStr);
						}
					});
				});
			});
