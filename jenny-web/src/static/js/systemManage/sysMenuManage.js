$(function() {
	...
	$('#saveSysRole').on('shown.bs.modal', function(event) {
		...
		var roleCode = button.attr('roleCode');
		var modal = $(this) //获得模态框本身
		modal.find('form input').each(function() {
			if(roleCode) {
				var row = $("#sysRoleTable").bootstrapTable('getRowByUniqueId', roleCode);
...
function operateFormatter(value, row, index) { //赋予的参数
	...
	if (!row.enable) {
		operate.push('<a roleCode="' + row.roleCode + '" class="btn active" data-toggle="modal" data-target="#saveSysRole" href="#">编辑</a>');
